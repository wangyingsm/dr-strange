...
|---|---|---|---|---|
| 按键点查 | 3.8 µs | 266.2 µs | **3.6 µs** | 299.7 µs |
| 一跳扩展 | **6.8 µs** | 1.71 ms | 9.0 µs | 381.9 µs |
| 两跳可达集 | **30.7 µs** | 7.19 ms | 67.4 µs | 931.9 µs |
| 向量 top-k 查询 | **306.2 µs** | 7.75 ms | — | 2.66 ms |

嵌入式 KV 设计将所有图查询保持在个位数到数十微秒——点查与 SQLite 持平，扩展与
多跳遍历为全场最快——而向量检索是其拉开差距之处：top-k 延迟约为 Neo4j 的 1/9、
Kùzu 的 1/25，索引构建速度也数倍于两者（完整表格见 BENCHMARKS.md）。批量加载仍
