...
|---|---|---|---|---|
| 按键点查 | 12.0 µs | 306.8 µs | **3.8 µs** | 783.0 µs |
| 一跳扩展 | 17.7 µs | 2.03 ms | **9.0 µs** | 504.5 µs |
| 两跳可达集 | **53.2 µs** | 8.94 ms | 69.5 µs | 1.08 ms |
| 向量 top-k 查询 | **320.5 µs** | 9.28 ms | — | 3.51 ms |

嵌入式 KV 设计将点查与图查询保持在微秒级——多跳遍历为全场最快——而向量检索是其
拉开差距之处：top-k 延迟比 Neo4j 低一个数量级、约为 Kùzu 的 1/30，索引构建速度
也数倍于两者（完整表格见 BENCHMARKS.md）。批量加载仍落后于成熟的列式引擎。数据
均为单次运行、预热后、单机测得，各引擎背靠背连续运行——**仅供参考，并非排行榜**。
方法学、注意事项、加载吞吐数据以及复现方式（`just bench-compare`，或仅测
dr-strange 的 `just benchmark`）详见 **[BENCHMARKS.md](BENCHMARKS.md)**。
