...
impl<'a> TreeAccess<'a> {
    ...
    fn plane_root(
        ...
        match self.attached {
            ...
            Some(_) => anyhow::bail!(
                "this plane's `synced_root` ({}) is not inside the tree attached                  to this server, which reads no other files; run `serve watch`                  on that tree or use the stdio server on your own machine",
                recorded.display()
            ),
            None => anyhow::bail!(
                "this server has no source tree attached and reads no local                  files — the plane's `synced_root` ({}) is readable only from                  `serve watch` on that tree or the stdio server",
                recorded.display()
            ),
        }
    }
...
fn contained(root: &std::path::Path, file: &str) -> AnyResult<std::path::PathBuf> {
    ...
    if rel.is_absolute() || escapes {
        anyhow::bail!(
            "refusing to read {file}: paths are relative to the source tree and              may not leave it"
        );
    }
    let root = root
        .canonicalize()
...
fn digest_logic(
    ...
    // Resolved before anything expensive, and before any key is read: a
    // disallowed provider is refused as a policy error, not after a
    // document was converted or a plugin ran.
    let (chat_provider, chat_key_env) =
        wire_provider(req.chat.as_deref(), req.key_env.as_deref(), allow)?;
    let (embed_provider, embed_key_env) = wire_provider(
        Some(req.embed.as_deref().unwrap_or(chat_provider)),
...
pub fn snippet_logic_in(
    ...
    let root = access.plane_root(plane_root.as_deref())?;
