...
`drsg` 是命令行界面。它**直接**打开数据库，属于嵌入式访问，而不是连到某个正在运行的
服务，因此适合本地脚本化、批量导入、检视、备份与运维。每次调用都接受一个全局的
`--db <路径>`（默认 `graph.drsg`）；多数命令还接受一个 `--plane`（默认 `startup`）。

由于它在进程内打开数据库，`drsg` 无法操作一个已经被 `drsg serve`（或另一个 `drsg`）
打开的数据库：数据库会取得一把排他锁，第二次打开会直接报错，而不会把数据库弄坏。并发
访问请改用面向该服务的 SDK 或 RPC 接口，命令行只用于离线操作。
...
表格与列表，而不是一堆散乱的字符。格式依据文件内容判断，扩展名写错也不影响。目录会
被逐文件遍历、分发：源码经已安装的解析器插件变成事实，文档则作为散文交给模型。
...
导入与导出都用 JSONL：每行一个节点或边，一个平面因此可以经由文件系统往返，也便于跟
其它工具集成：
...
既有数据上。边本身不带键，所以总是追加写入。这一策略约束的是节点身份，而不是边的
去重。没有 `external_key` 的行不可能发生冲突，始终会被插入。
...
以 `http(s)://` 开头的参数会被抓取，而不是从磁盘读取：页面会被转换为 Markdown，并在
`--pages`/`--depth` 范围内沿链接继续抓取，只保留与页面自身主题（以及给出的
`--topic`，如果有）相关的内容（[第 3 章](./ai-native.md#从-url-读取)）。协议头是
必需的：裸的 `example.com` 也可能是一个合法的文件名，与其去猜使用者的本意，不如
要求写明。命令会打印保留了什么、丢弃了什么，但不提供交互式挑选，那是仪表盘页面
列表要做的事。

`--mode` 决定抽取结果会被清理到何种程度：`coarse` 归一标签与边类型的词表，`fine`
（默认）在此之上合并指称同一事物的实体，`super` 再把每个实体连同其全部段落一并
重读，结果最准确，但输入 token 用量约为 15 倍（[第 3 章](./ai-native.md#抽取精度)）。
