...
事实能为编码智能体带来什么——以及，对于官方目录尚未覆盖的语言，如何构建你自己的
插件。
...
于是结构性问题变成**一次往返**，而不是一轮搜索加阅读的循环。

七个动词承担主要工作，在 MCP（[第 8 章](./mcp.md)）与 CLI
（[第 7 章](./embedded-cli.md)；`grep` 与 `snippet` 需要读取被监视的源码树，
因此只随服务端提供）上完全一致：
...
每个回答都是紧凑的每行一条事实的文本，按模型的上下文窗口而非终端来定尺寸；
`context` 通过收紧各分组的条数上限把自己保持在固定预算之内，并写明省略了什么。

在 `serve watch` 之下，图跟踪每次提交：变更的文件重新经过插件，平面就地更新——
新符号创建、消失的删除、边被改写——收敛到与一次全量重新图化完全相同的结果。每个
回答都以 `synced: commit <sha>` 开头，智能体因此知道自己推理的是*哪一版*代码；
工作区里未提交的修改在提交之前不可见，而回答通过注明提交号把这一点说在了明处。
...
- **调用清单是一个明示的下界。** 解析器没能解析的调用以带原因的 `UnresolvedRef`
  条目存在，直接呈现在 `context` 里——于是"谁调用了它"带着未解析的残余一起返回，
  而不是被悄悄缩短。
- **图在回答之内点明自己的盲区**：诚实脚注、`synced:` 行、省略计数。智能体可以
  自行决定何时回落到 `grep`——而 `grep` 动词就在同一套工具上，回落只是多一次
  往返，不用换工具。

在与 ripgrep 工作流以及两款开源代码图 MCP 工具的基准对比中——同样的语料、同样的
任务、每个智能体只用一种工具——这套组合完成了每一种任务形态（调用者、影响面、
调用链、复合审计），每项任务 2–4 次工具调用，边际 token 开销最低，且是唯一在
回答中明示自身边界的工具。方法与完整表格见
...
遵循、也最值得新插件借鉴的模式是：包装一个**成熟的、最好是该语言公认的解析器**
（syn、swc、ruff、tree-sitter），而不是自己写一个；并把解析器保持为普通的原生库，
外面只套一层薄薄的组件封装，这样它的测试完全不需要 wasm 工具链。
...
真正的语言解析器直接实现生成的 `Guest` trait——`parse` 每块返回一个序列化的部分
结果，`assemble` 在全部部分结果之上做跨文件解析——并通过宿主绑定拉取相邻文件。
官方的 `plugins/rust` 就是现成的完整范例。
...
这些构建参数每个都有其作用（扩展仓库的 `justfile` 解释了原因）。Go SDK 里贯穿
一条规则：凡是从 ABI 提升上来的数据，先复制再使用——`cm` 切片只是一个视图，
垃圾回收器可能在你使用时把底下的数据挪走。
...
想把它提供给所有人：贡献从在[扩展仓库](https://github.com/wangyingsm/dr-strange-extension)
开一个 issue 开始，写明你打算包装的解析器；以 `plugins/<名称>/{parser,component}`
的形式落地，并附带原生测试套件——CI 会在每次推送时构建所有组件并运行所有测试。
