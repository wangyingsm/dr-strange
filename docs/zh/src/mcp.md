...
器重启它的 MCP 子进程——什么都不会发，服务端改用定时器回收：**空闲 5 分钟**，或会话创
建后 **60 秒**内没有收到 `initialize`。该会话的 worker task、它的 `DrStrange`、以及缓
冲的消息都随之释放。

残留的只有每个死会话一条 map 条目——一个会话 id 加一个已关闭的句柄，几十字节，不会被
复用。对于连续运行数月、host 每天崩溃几次的服务器，这是噪声,不值得为它重启；但如果你
在脚本里循环创建会话,记得关闭,残留就恒为零。
