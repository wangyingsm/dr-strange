...
| `history` | 读 | 一个仓库历史的概览——HEAD 在哪、分支与标签指向何处、哪些分支被变基过，以及最新的若干提交；读的是 git 检出目录被图化时一并写入的 `_git` 平面 |
| `recall` | 读 | 某个 git 修订版本时的代码——一个文件、一个行区间、一个目录，或一个符号当时的声明（按当时的文件重新解析得到）；`pattern` 在那棵树上检索，`vs` 给出一个名字相对更早修订版本的差异；修订版本经由 `_git` 平面解析 |
| `traverse` | 读 | 从某个节点进行邻域扩展（1 跳及以上） |
| `query` | 读 | 运行一个序列化的逻辑计划 |
| `cypher` | 读 / 写 | 运行一条 openCypher 子集语句——任何具名动词都未预见的问题，由它兜底 |
