...
目录那样逐级向上，到仓库根（第一个 `.git`）为止；不属于当前用户或任何人可写的文件会被
跳过，因为它指名的服务将接收宿主的会话及其携带的 `Authorization`——若其中声明了一个
drsg 服务且该服务有应答，本进程便把宿主的会话
...
借此削弱针对本机服务的 DNS 重绑定攻击，对不认识的名字返回 **403**。回环地址
（`localhost`、`127.0.0.1`、`::1`）始终放行。配置了 bearer token 之后，服务端绑定
的地址也会放行，`DRSG_ALLOWED_HOSTS`（逗号分隔，主机名或 `host:port`）里的每个
名字同样放行——所以把 `/mcp` 放到 `https://memory.example.com` 后面，只需
`DRSG_ALLOWED_HOSTS=memory.example.com`。未设 token 时，无论环境变量怎么写，列表
都只有回环地址：未设 token 的服务端信任自己的同源 UI，而重绑定页面要冒充的正是
这个身份，`Host` 检查就是用来挡它的。有了 token，每个请求都必须携带这种页面读不
到的秘密，这道检查就不再比 token 多挡住什么，运营者可以放宽它。ROADMAP §10 针对
的场景——同一台机器上的多个智能体宿主——什么都不用配。
...
失败：服务器繁忙时该让 agent 等，而不是把它拒之门外——但不会无限期地等：每次
调用有一个覆盖排队与执行的截止时间，默认 **300 秒**，可用
`DRSG_MCP_TOOL_DEADLINE_SECS` 调整（`0` 关闭）。超时的调用以工具级错误返回，并说明
是服务器繁忙还是执行过慢；已在运行的调用体不会被中断，但会一直占着它的名额直到
结束，所以上限仍然算得上它。
...
| `grep` | 读 | 在被监视的源码树上做文本检索——字面或正则，可用 `path` 限定范围并带上下文行；每个命中都标出所在的符号 |
| `trace` | 读 | 两个符号之间图中记录的最短调用路径 |
| `impact` | 读 | 所有能到达该符号的东西，按距离分组 |
| `fathom` | 读 | 一个符号周边区域的构成——标签、边类型与枢纽 |
| `snippet` | 读 | 一个符号的源码文本，或文件的某个行区间（`path:start-end`）；会说明区间落在哪个符号里、如何继续往下读 |
| `traverse` | 读 | 从某个节点进行邻域扩展（1 跳及以上） |
| `query` | 读 | 运行一个序列化的逻辑计划 |
| `cypher` | 读 / 写 | 运行一条 openCypher 子集语句——任何具名动词都未预见的问题，由它兜底；`DELETE`/`REMOVE` 需要 `confirm: true` |
...
| `digest` | 写 | 导入一篇文档（默认为 dry-run；`apply: true` 需要 `confirm: true`；`mode` 决定抽取精度） |
...
- **破坏性写入受到保护，增量写入则不受。** `drop_plane`、带 `DELETE` 或 `REMOVE`
  的 `cypher` 语句、以及 `apply: true` 的 `digest` 都需要 `confirm: true`，缺少时会
  拒绝并指出该标志；`digest` 默认为 dry-run，返回拟议的节点与边以供检视，而非直接
  写入。`write_nodes`、`write_edges` 和 `CREATE`/`MERGE`/`SET` 无需确认：它们添加的
  内容可由之后的删除撤销，而一个事事都要的标志只会被客户端不假思索地传上。
- **文件只从源码树内读取。** `grep` 与 `snippet` 打开的每个路径都必须解析到它所属
  的树之内：`..`、绝对路径、指向树外的符号链接一律拒绝。操作者挂接的树（`serve
  watch` 的目录或 `[server] source_root`）始终可读；平面自身的 `synced_root` 是
  数据，stdio 服务以你的身份运行、可以照读，而共享的 `drsg serve` 只在它位于挂接
  的树内时才读；未挂接任何树时，`/mcp` 不读任何文件。
- **镜像平面不接受手工改写。** 由 `digest` 或 `serve watch` 与源码树保持同步的
  平面记录着它所反映的提交，`cypher` 在这类平面上拒绝
  `CREATE`/`MERGE`/`SET`/`REMOVE`/`DELETE`：下一次折叠会把平面与源码树重新对
  账，覆盖对解析器所有节点的修改、重建被删除的节点，这样的写入会被无声地撤销。
  `write_nodes` / `write_edges` 在那里仍然可用——折叠不会动它不拥有的节点——
  而智能体自己的平面接受任何语句。

在同样这些镜像平面上，`cypher` 对 `RETURN n` 以智能体动词共用的紧凑文本作答——
一个计数、同步的提交、每个节点一行——而不是每个节点一条 JSON 记录；投影查询在
任何平面上都返回表格。语句解析失败时，返回会附上它所触及的那几条子句的语法，
因此工具清单只需承载意图与两个例子，而不必每一轮都携带整门语言。
