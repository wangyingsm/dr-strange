...
```console
$ drsg update
drsg 2.13.0 is the latest release — nothing to do
```
...
$ drsg update
drsg 2.12.0 -> 2.13.0
$ curl -fsSL https://raw.githubusercontent.com/wangyingsm/dr-strange/v2.13.0/scripts/install.sh | sh -s -- --bin 'drsg' --version 'v2.13.0' --dir '/home/me/.local/bin'
Dr Strange v2.13.0 (x86_64-unknown-linux-gnu)
  downloading dr-strange-v2.13.0-x86_64-unknown-linux-gnu.tar.gz
  checksum verified
  installed /home/me/.local/bin/drsg
```

两个阶段都要穿过网络，也都遵循代理设置：版本检查是 drsg 自己发出的请求（参见
[在 HTTP 代理之后工作](#在-http-代理之后工作)），而它随后交棒的安装脚本由 `curl`
执行，读取的是同一批变量。在需要代理的网络里，最先失败的是版本检查，因此一次
够不到 GitHub 的 `update` 根本走不到安装脚本那一步。
...
[network]                                   # 本程序自身发出的请求走向何处
proxy = "http://127.0.0.1:7897"             # 也可为 socks5://…；被 ALL_PROXY / HTTPS_PROXY / HTTP_PROXY 覆盖
no_proxy = ".internal, 192.168.1.10"        # NO_PROXY 形式的绕行清单；被 NO_PROXY 覆盖
```
...
`[network]` 管辖的是 drsg **代表你**发出的请求：`drsg plugin install`、
`drsg update`、`drsg digest <url>`、LLM 提供方，以及副本拉取快照。可以写在配置
文件里，也可以导出 `ALL_PROXY`、`HTTPS_PROXY` 或 `HTTP_PROXY` 中的任意一个（每个
同时按小写形式读取）。`socks5://` 与 `http://` 同样受支持。

其中有三条规则值得了解，因为每一条都是经过权衡的决定，而非偶然：

**由调用方指定的 URL 绝不走代理。** JSON-RPC 上的 `/digest/fetch` 与
`plugin.install` 接受调用方给出的 URL，而上文所述的地址防护正是用来阻止这个 URL
指向服务端自身的回环地址或云元数据端点。代理会代替服务端解析名字并建立连接，防护
因此再无地址可供判断——它在代理之下无法生效，drsg 也不会假装它仍然生效。这些请求
保持直连并受防护约束。当配置了代理时，`drsg serve` 会在启动时说明这一点。

**回环地址永不走代理**，无论如何配置。本机模型服务——`ollama` 预设即
`http://localhost:11434/v1`——并不是任何人为访问外网而设置 `https_proxy` 时想要
改道的目标。除此之外的地址一律遵循配置；只有 `localhost` 与回环网段是自动的。

**环境变量优先。** 已设置的变量会覆盖配置文件，因此无需改动任何文件即可让单条命令
走另一个代理：

```bash
https_proxy=http://127.0.0.1:1080 drsg plugin install ts   # 仅本次运行
https_proxy= drsg plugin install ts                        # 已设置但为空：直连
```

`no_proxy` 接受 `NO_PROXY` 形式的清单，以逗号或空格分隔：精确主机名、`.suffix`
或 `*.suffix` 通配、IP 字面量、带端口的条目（`example.com:8080`），或用 `*` 表示
全部。不支持 CIDR 网段。环境中的 `NO_PROXY` 会**替换**配置文件中的清单，而不是与
之合并。
