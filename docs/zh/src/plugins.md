...
数据库发版，数据库发版也不必等十条工具链都跟上。这个仓库就是扩展的公共地：官方
...
过去）；根目录之内、但 `%list` 不会列出的路径同样被拒绝——`.gitignore`、隐藏文件与
构建目录的规则对两者一视同仁，所以插件不能靠直接点名去读 `.env` 或被忽略的凭据
文件。`label` 在输入内容无法自述名字时提供一个名字。
...
- 解析得到的事实携带 `_generated_by`（如 `rust@3+fdb01bb6`：插件名、版本，加上产出
  这条事实的构件——后缀是该 wasm 已钉住的 SHA-256 的前几个字节，因此同一版本插件的
  不同字节产出的事实不会被读成同一来源）而非 `_model`，因此永远能与
...
一次 `parse` 调用只处理**一个文件**，所以插件在某个文件上栽了跟头，损失也就是这
一个文件：它会被跳过并计数，报告里会点名。最常见的元凶是生成代码——`.pb.go` 里那个
由上千段字符串拼接而成的描述符，会让递归下降的打印器一路走出插件链接时定下的栈；
这是插件作者要修的，任何宿主配置都抬不高那个栈。但如果插件在它认领的**每一个**文件
上都失败，那就是另一回事了，这一趟仍然会被判定为失败。

**`assemble` 没有这层边界，这个差别值得记住。** 它一次性地在全部 partial 上运行
一遍，因此那里的一次 trap 不是一个文件的损失：它会让整个插件失败，连带正在折叠这
棵树的那趟分发一起失败。于是，一个在仓库中占比微不足道的语言里的单个文件，就足以
让平面变空——而在 `drsg init --rebuild` 下（它会先丢弃平面再重新折叠），这是破坏性
的，而非一次无操作。在宿主把出错插件与整趟分发隔离开之前，请把 `assemble` 的失败
读作一次故障，而不是报告里的一处缺口：操作者的应对是先装回该插件的上一个版本，等
解析器修好。
...
十个官方插件覆盖常用语言与两类横切来源，每个都包装一个成熟的解析器而非重新发明：
...
| `git` | *（一个 git 检出目录）* | 直接读取 git 自己的对象库，用 [flate2](https://crates.io/crates/flate2) 解 zlib 流。它按“来源是一个检出目录”分发，而非按扩展名；写出的历史平面就放在代码平面旁边 |
| `deps` | *（清单文件，按文件名）* | [toml_edit](https://crates.io/crates/toml_edit) 与 [quick-xml](https://crates.io/crates/quick-xml)：`package.json`、`go.mod`、`pyproject.toml`、`requirements.txt`、`pom.xml`。清单是一个*文件名*而非扩展名——`Cargo.toml` 仍归 `toml` 插件 |
...
安装还会把插件编译一次，并将编译产物保存在 wasm 旁边，以它自己的哈希固定。此后
每次加载——每次 `digest`，`serve watch` 折叠的每个提交——都在毫秒内反序列化这份
产物，而不再重新编译；对官方解析器而言，编译意味着数秒 CPU 和数百 MiB 内存。
编译产物只属于生成它的那个 drsg 构建：`drsg update` 之后，或在它出现之前建立的
插件库中，第一次加载仍会像以前一样编译，并把结果保存下来供下一次使用。
