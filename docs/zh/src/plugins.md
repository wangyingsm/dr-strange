...
`drsg digest` 遍历一个代码仓库时，不会让模型去猜测代码的结构。每个源码文件被分发给
一个**预处理插件**——一个沙箱化的 WebAssembly 组件，用编译器级的解析器解析它，并
返回**事实**：解析器确信无疑的节点与边。AST 不需要推断 `parse()` 调用了 `lex()`，
它就是知道。

插件位于独立的仓库
[dr-strange-extension](https://github.com/wangyingsm/dr-strange-extension)——
与数据库分开是有意为之：官方并不意味着步调锁死。解析器修一个缺陷不必等数据库发版，
数据库发版也不必等八条工具链。这个仓库就是扩展公共地——官方插件、契约的规范副本，
以及编写自己插件所需的 SDK（后者是[编码智能体](./coding-agent.md)一章后半部分的
主题）。
...
`%list` 返回被消化根目录下可读的路径，**已排序**——未排序的目录顺序会让两次运行
...
的输入数，外加用文字写明的注记：什么没能解析、为什么。计数并点名，而不是悄悄丢弃：
...
一个插件，而且**由沙箱强制执行，而非仅仅要求作者自觉**——没有网络（`wasi:sockets`
在加载时按名拒绝）、没有环境变量、除三个宿主函数外没有文件系统，插件根本无从触及
任何模型服务，也拿不到调用所需的密钥。

分工正是意义所在。插件的职责是解析器能*证明*的东西，且是确定性的；真正需要模型的
内容——一段文档注释的含义、一份 README、任何解析器无法断言为事实的东西——作为
...
- 确定性是契约的一部分，不是一个愿望。沙箱冻结时钟、按固定序列发放熵、对目录列表
  排序——同一棵树消化两次，得到逐字节相同的事实。插件内部的模型调用恰恰会破坏
  这一点。
...
插件在沙箱里崩溃时，它写到 stderr 的内容会一并收进报给操作者的错误信息，方便定位
问题。插件的全部产出都以**返回值**的形式交回宿主——写数据库的，永远只有宿主。
...
`<plugin>.wasm` 与其 SHA-256。drsg 的二进制固定这份目录——发布标签 URL 加哈希，
即与该构建的契约验证兼容的版本——不带参数的 `drsg plugin install` 会交互式列出
它。安装时固定制品的 SHA-256，之后每次加载都重新校验，磁盘上被改动的文件会被
拒绝而不是被悄悄运行。对同名插件再次安装即是升级路径。
