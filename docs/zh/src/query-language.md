...
字符串可用任一种引号，并支持常见转义——`\'`、`\"`、`\\`、`\n`、`\t`、`\r`、
`\b`、`\f`、`\uXXXX`——因此含引号的值写在字面量之内，而不会终止它；未知转义或未闭合的字符串
...
时间旅行的读取无法使用向量索引与关键词索引，因为它们只按最新提交构建；这类相似度
检索转而扫描被钉住的快照，关键词与混合检索则直接在该快照上计算 BM25——结果依然正确
（匹配与得分都是快照自身的，此后被删除或修改的节点仍按当时的样子被找到），只是不经
索引加速。时间旅行需要原生后端。
