...
in §9 and to preprocessors in §11) · last revised 2026-08-12
...
| Preprocessing | an input's own structure → **facts** (nodes and edges a parser is certain of) plus **prose** (the residue needing a model), routed per file so a polyglot tree fans out and merges (ROADMAP §11). Document reading is the fallback every unclaimed input lands on, so a default install is unchanged. An input yielding only facts is digested with **no model call at all**. Local-only: the CLI and the stdio MCP server, never a shared server — see §2 |
